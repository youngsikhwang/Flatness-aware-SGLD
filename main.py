...
def get_optimizer(model, optimizer_name: str, lr: float, **kwargs):
    ...
    if optimizer_name == 'sgd':
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=kwargs.get('milestones',[100,150]), gamma=0.1)
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(base_opt, milestones=kwargs.get('milestones',[100, 150]), gamma=0.1)
    else:
...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch, args):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        if args.optimizer in ['fgld', 'sam', 'sgld']:
...
def main():
    parser = argparse.ArgumentParser(description='Noisy Label Training with fGLD and SAM')
    
    # Model settings
    parser.add_argument('--backbone', type=str, default='resnet34', 
                       choices=['resnet34', 'resnet50', 'vit'], help='Backbone architecture')
    
    parser.add_argument('--dataset', type=str, default='cifar10N',
                       choices=['cifar10N', 'cifar100N', 'animal10N', 'clothing1M', 'webvision'], 
                       help='Dataset name')
    parser.add_argument('--data_root', type=str, default='./data', help='Data root directory')
    parser.add_argument('--train_file', type=str, default=None, 
                       help='Train dataset file name (default: {dataset}_train.npz)')
    parser.add_argument('--test_file', type=str, default=None,
                       help='Test dataset file name (default: {dataset}_test.npz)')

    # Training settings
    parser.add_argument('--optimizer', type=str, default='sgd',
                       choices=['sgd', 'adam', 'fgld', 'sam'], help='Optimizer')
    ...
    # fGLD specific
    parser.add_argument('--sigma', type=float, default=0.1, help='fGLD perturbation scale')
    parser.add_argument('--n_pert', type=int, default=1, help='fGLD number of perturbations')
    parser.add_argument('--beta_inv', type=float, default=1e-14, help='fGLD Langevin noise scale')
    parser.add_argument('--pert_type', type=str, default='normal',
                       choices=['normal', 'antithetic'], help='fGLD perturbation type')
    ...
    if args.optimizer == 'fgld':
        exp_name += f"_sigma{args.sigma}_npert{args.n_pert}_pertype{args.pert_type}"
    elif args.optimizer == 'sam':
        exp_name += f"_rho{args.rho}"
        if args.adaptive:
            exp_name += "_adaptive"
    exp_name += f"_lr{args.lr}_bs{args.batch_size}_seed{args.seed}_momentum{args.momentum}"
   ...
   # Load dataset
    train_dataset, test_dataset, num_classes = get_dataset(
        args.dataset, args.data_root, args.train_file, args.test_file
    )
