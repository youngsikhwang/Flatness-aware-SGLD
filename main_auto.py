...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch, optimizer_name):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        if optimizer_name in ['fgld', 'sam', 'sgld']:
...
def objective(trial: optuna.Trial, args):
    ...
    elif args.optimizer == 'fgld':
        # fGLD: tune lr, sigma, beta_inv, momentum
        lr = trial.suggest_float('lr', 0.01, 1.0, log=True)
        optimizer_kwargs['sigma'] = trial.suggest_float('sigma', 1e-3, 1e-1, log=True)
        optimizer_kwargs['n_pert'] = 1  # Fixed
        optimizer_kwargs['beta_inv'] = 1e-14
        optimizer_kwargs['pert_type'] = 'normal'  # Fixed
        optimizer_kwargs['momentum'] = trial.suggest_float('fgld_momentum', 0.0, 0.9)

    elif args.optimizer == 'sgld':
        # SGLD: tune lr, momentum
        lr = trial.suggest_float('lr', 0.01, 1.0, log=True)
        optimizer_kwargs['momentum'] = trial.suggest_float('momentum', 0.0, 0.9)
        optimizer_kwargs['beta_inv'] = 1e-14

    elif args.optimizer == 'sam':
        # SAM: tune lr, rho, momentum
        lr = trial.suggest_float('lr', 0.01, 1.0, log=True)
...
def run_optuna_optimization(args):
    ...
    if args.visualize:
        try:
            ...
            fig.write_html(os.path.join(args.save_dir, f"{study_name}_history.html"))
            
            # Parameter importance
            if len(completed_trials) >= 10:
                fig = plot_param_importances(study)
                fig.write_html(os.path.join(args.save_dir, f"{study_name}_importance.html"))
...
def train_with_best_params(args, best_params):
    ...
    optimizer_kwargs = {
        'weight_decay': weight_decay,
        'milestones': [int(args.final_epochs * 0.25), int(args.final_epochs * 0.5)],
        'gamma': 0.1
    }
    ...
    if args.optimizer == 'sgd':
        ...
        optimizer_kwargs['pert_type'] = 'normal'  # Fixed
        optimizer_kwargs['momentum'] = best_params.get('fgld_momentum', 0.9)
...
def main():
    ...
    parser.add_argument('--optimizer', type=str, default='sgd',
                       choices=['sgd', 'fgld', 'sam', 'sgld', 'adamw'], 
